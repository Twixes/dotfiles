...
description: >
  ...
  needs to know about their product right now — and save it as a fresh PostHog Notebook.
  It reads the user's PostHog project (analytics, the Inbox / Signals, error tracking,
  experiments and feature flags, and more) plus their other connected tools (GitHub,
  Slack, calendar…), figures out what actually matters to *this* user today, and ranks it
  into a prioritized action list backed by evidence. Use this whenever the user wants to
  catch up on the state of their product, or says things like "today in product", "what
  should I know today", "what needs my attention", "what's on fire", "catch me up on my
  product", "daily product briefing", "product standup", "what changed overnight", "morning
  briefing", or asks for a rundown of errors + experiments + inbox + metrics together. Also
  trigger it as a recurring morning ritual (it pairs with /loop and /schedule). Reach for
  this even when the user names only one pillar ("any new errors today?") but clearly wants
  the day's picture — the skill covers that pillar in full and surfaces anything else urgent.
---

# Today in Product

A product engineer starts the day wanting one thing: **what deserves my attention today, and why.**
Not a dashboard to interpret — a briefing. This skill reads across their PostHog project and
their other tools, decides what genuinely matters *to them today*, ranks it, and writes it into
a dated PostHog Notebook they (and their team) can open, trust, and act from.
...
1. **The Notebook — the record.** A new PostHog Notebook per day, titled `Today in Product — <weekday>, <Mon D>`.
...
- **Project & identity:** use the PostHog MCP active project and the logged-in user. The MCP context
  already tells you the active project id, base URL, and the user's name/email — use them. If the user
  named a different project in their prompt, honor that instead. If no PostHog project is reachable at
  all, say so plainly and stop — this skill is built around PostHog data.
...
- **Deep links:** don't hardcode `app.posthog.com`. Resolve correct URLs with the PostHog `generate-app-url`
  MCP tool (or reuse the base URL from MCP context) so links work on US, EU, and self-hosted instances.
...
**invoke the specialist PostHog skill** rather than reinventing its queries — they encode the right
...
Create a **new** PostHog Notebook (don't update yesterday's) titled `Today in Product — <weekday>, <Mon D>`.
...
<PostHog Signals reports for this user, terminal state / awaiting input; each linked>
...
- Then, for the items where a **low-risk PostHog action** is available, offer to do it — one at a time,
...
- Anything that posts outside PostHog (a Slack message, a GitHub comment) unless the user asked for it.
...
| **Analytics** | Core-flow metrics vs. prior period: activation, key funnel steps, retention, signups, revenue. Find *movements*, not vanity totals. Explain the likely why (tie a drop to an error/flag if you can). | `investigate-metric`, `investigating-metric-anomalies`, `querying-posthog-data`, `visualizing-change-over-time` | "Checkout funnel −8% at payment since 2pm, tracks a new Stripe error → investigate" |
| **Errors & perf** | New issues, and issues whose volume is spiking vs. baseline; assigned-to-me first. Notable APM / web-vitals regressions. | `triaging-error-issues`, `investigating-error-issue`, `grouping-noisy-errors`, `exploring-apm-traces` | "Error PH-… +400% since a deploy 3h ago, hits 1.2k users → triage" |
...
## Sources beyond PostHog — use everything available, degrade gracefully
...
a public REST API over WebFetch as a last resort** — and skip any tool that isn't set up (a friend running
this may have none of them; that's fine, the PostHog picture stands on its own).
...
When you use a non-PostHog source, keep it in service of the product picture: it earns a place only when
...
- **Portability first.** Nothing here should assume PostHog-employee tooling or a specific company. It must
  work for any product engineer on any PostHog project. When a tool is missing, degrade silently — never
...
- **The record doesn't act.** Re-emphasized because it's the core contract: the notebook only links; any
  change to PostHog happens in the chat, on an explicit yes, and never for flags/experiments.
